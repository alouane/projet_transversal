...
function removeUser() {
	var curtain = MM_findObj('removeUser');

	var infowin = MM_findObj('infowin');
	MM_findObj('removeFullName').innerHTML = infowin.username;

	curtain.style.height = windowheight + "px";
	curtain.style.display = "block";
	if (animations) {
		new Rico.Effect.FadeTo('removeUser', .85, 300, 5, {} );
	} else {
		new Rico.Effect.FadeTo('removeUser', .85, 0, 1, {} );
	}
}
